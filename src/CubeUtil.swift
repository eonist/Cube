...
public class CubeUtil{
   ...
   public static func cube(quad:Quad) -> Cube{
      ...
      let ratio = CubeUtil.ratio(vp1: vp1, vp2:vp2, quad: quad, quadCenter: quadCenter)
      let vp3:CGPoint = CubeUtil.vp3(vp1: vp1, vp2: vp2, quadCenter: quadCenter,quad:quad)
...
extension CubeUtil{
    ...
    * vp1
    */
   static func vp1(quad:Quad,quadCenter:CGPoint)->CGPoint{
   ...
   //use quad center when finding infinite vp2, and double check tht it works
   /**
    * vp2
    */
   static func vp2(quad:Quad,quadCenter:CGPoint)->CGPoint{
    ...
    * vp3
    */
   static func vp3(vp1:CGPoint,vp2:CGPoint,quadCenter:CGPoint,quad:Quad) -> CGPoint{
...
extension CubeUtil {
    ...
    */
   static func ratio(vp1:CGPoint, vp2:CGPoint,quad:Quad, quadCenter:CGPoint ) -> (inward:CGFloat,outward:CGFloat){
...
//      Swift.print("closestVP:  \(closestVP)")
      //THIS NEEDS BETTER CODE, you have to figure out which 2 points are closer to vp, but less complicated than this
      let side:(CGPoint,CGPoint) = {
         if vp1Dist <= vp2Dist {
            let ver1Center:CGPoint = CGPoint.interpolate(quad.p1, quad.p3, 0.5)
            let ver2Center:CGPoint = CGPoint.interpolate(quad.p2, quad.p4, 0.5)
            ...
            let hor1Center:CGPoint = CGPoint.interpolate(quad.p1, quad.p2, 0.5)
            let hor2Center:CGPoint = CGPoint.interpolate(quad.p3, quad.p4, 0.5)
...
//      Swift.print("side:  \(side)")
      let halfSidePT:CGPoint = CubeHelper.convergingPoint(a: (closestVP,quadCenter), b:side)
//      Swift.print("halfSidePT:  \(halfSidePT)")
      let vpLength:CGFloat = CGPointParser.distance(quadCenter, closestVP)
...
class CubeHelper{
   ...
   static func center(quad:Quad) -> CGPoint{
      let x = CGPoint.interpolate(quad.p1, quad.p2, 0.5).x
      let y = CGPoint.interpolate(quad.p1, quad.p3, 0.5).y
